...
export class Product {
  constructor(raw, index) {
    // [ЗАСВАР] raw.id байвал түүнийг ашигла, үгүй бол index+1
    ...
    this.desc = raw.desc || raw.description || "";

    // [НЭМЭЛТ] publish request эсэхийг тэмдэглэх
    this.type = raw.type || "product";
  ...
  static guessCategory(name, brand) {
    ...
    if (
      /dress|gown|ballroom|tango|wedding|kimono|violet|evergarden|anna|blue/.test(
        n,
      )
    )
      return "dress";
  ...
  get imgSrc() {
    if (!this.img) return "";
    if (this.img.startsWith("http") || this.img.startsWith("/"))
      return this.img;
    return "/public/source/" + this.img;
  }
  ...
  /* [НЭМЭЛТ] Publish request эсэх */
  get isPublishRequest() {
    return this.type === "publish_request";
  }
}
...
export class User {
  ...
  get statusLabel() {
    return this.isActive ? "Идэвхтэй" : "Идэвхгүй";
  }
  get statusClass() {
    return this.isActive ? "active" : "rejected";
  }
}
