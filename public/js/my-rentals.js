...
var defaultRentals = [
  {
    ...
    brand: "Hugo Boss",
    size: "S",
    img: "/public/source/mongol-deel.webp",
  ...
  },
  {
    ...
    brand: "Belucci",
    size: "XS",
    img: "/public/source/big-blue-dress.webp",
...
function renderActiveRentals() {
  ...
  if (activeItems.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-icon">📦</div><p>Идэвхтэй захиалга байхгүй</p></div>';
    return;
  }
  ...
  container.innerHTML = activeItems
    .map(function (r) {
      ...
      if (r.status === "pending") {
        actionBtn =
          '<button class="btn-sm success btn-accept" data-id="' +
          r.id +
          '">✓ Хүсэлт зөвшөөрөх</button>';
      }
      if (r.status === "delivery") {
        actionBtn =
          '<button class="btn-sm success btn-confirm" data-id="' +
          r.id +
          '">Хүргэлт баталгаажуулах ✓</button>';
      }
      if (r.status === "active" || r.status === "overdue") {
        actionBtn =
          '<button class="btn-sm danger btn-return" data-id="' +
          r.id +
          '">Буцааж өгсөн ✓</button>';
      }
      ...
      return (
        ...
        '<p class="p-order-name">' +
        r.name +
        "</p>" +
        '<p class="p-order-meta">' +
        r.brand +
        " · " +
        r.size +
        " · " +
        ...
        " өдөр</p>" +
        "</div>" +
        '<strong class="p-order-total">' +
        fmt(r.price) +
        "</strong>" +
        '<div class="p-order-right">' +
        '<span class="p-order-status ' +
        color +
        '">' +
        label +
        "</span>" +
...
function acceptRequest(id) {
  var rental = rentals.find(function (r) {
    return r.id === id;
  });
  if (!rental) {
    return;
...
function addRentalToCart(rental) {
  ...
  var alreadyIn = cartItems.some(function (i) {
    return i.id === rental.id;
  });
  if (alreadyIn) {
    return;
  }
...
function renderHistoryRentals() {
  ...
  if (historyItems.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-icon">🕐</div><p>Захиалгын түүх байхгүй</p></div>';
    return;
  }
  ...
  container.innerHTML = historyItems
    .map(function (r) {
      ...
      if (r.status === "done" && !r.reviewed) {
        reviewBtn =
          '<button class="btn-sm success btn-review" data-id="' +
          r.id +
          '">★ Сэтгэгдэл бичих</button>';
      }
      if (r.status === "done" && r.reviewed) {
        reviewBtn =
          '<span style="font-size:.7rem;color:var(--gold)">★ Бичигдсэн</span>';
      }

      return (
        '<div class="p-order">' +
        '<img class="p-order-img" src="' +
        r.img +
        '" alt="' +
        r.name +
        '">' +
...
function updateStats() {
  ...
  if (el1) {
    el1.textContent = activeCount;
  }
  if (el2) {
    el2.textContent = rentals.length;
  }
}
...
function submitReview() {
  ...
  if (selectedStars === 0) {
    showToast("Одны үнэлгээ сонгоно уу!", "red");
    return;
  }
  if (!comment) {
    showToast("Сэтгэгдэл бичнэ үү!", "red");
    return;
  }

  var rental = rentals.find(function (r) {
    return r.id === currentReviewId;
  });
  if (rental) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var hashMap = {
    ...
    incoming: "tab-active",
    notifications: "tab-info",
  };
  if (hashMap[hash]) {
    switchToTab(hashMap[hash]);
  }
});
